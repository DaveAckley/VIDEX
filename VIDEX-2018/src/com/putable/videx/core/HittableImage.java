...
public class HittableImage {
    ...
    public boolean isAlphaHittable() {
        return mAlphaHittable;
    }
    public boolean isLastLoadGood() {
    ...
    public void setImageFromBytesIfPossible(String sourceLabel, byte[] bytes) {
        try {
            ...
            g2d.drawString("IMAGE DATA READ FAILED FOR: " + sourceLabel, 10,
                    10);
            mLastLoadGood = false;
        }
    ...
    public void drawImage(VOGraphics2D v2d, int x, int y) {
        if (v2d.isRenderingToHitmap() && !this.isAlphaHittable()) {
            v2d.getGraphics2D().fillRect(x, y, mImage.getWidth(), mImage.getHeight());
        } else {
            BufferedImage bi = getImage(v2d);
            v2d.getGraphics2D().drawImage(bi, x, y, bi.getWidth(),
                    bi.getHeight(), null);
        }
